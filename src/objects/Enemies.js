...
class Enemy extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(scene, x, y, texture, yLevel, index, enemyGroup) {
        ...
        this.isDiving = false;  //isDiving to avoid calling startDiving on an already diving enemy
        this.prevX = x;
        this.prevY = y;
        this.yLevel = yLevel;
        this.index = index;
    ...
    shoot(bullets) {
        ...
        const difficultyFireRateFactor = 1 + this.enemyGroup.difficulty * 0.05; // Increase fire rate by 5% per difficulty level
        const currentFireRate = this.fireRate / difficultyFireRateFactor;
        if (this.active && currentTime > this.lastFired + currentFireRate) {
    ...
    //startDive defines the diving path and function
    startDive() {
        this.isDiving = true;
        var diveDuration;
...
export default class EnemyGroup extends Phaser.Physics.Arcade.Group {
    constructor(scene) {
        ...
        this.verticalGap = 30;
        this.timeElapsed = 0;
        this.isbreathing = false;   // Disabled breathing for testing
